...
const API_URL =
  ...
  "https://ai-marketplace-monetization.onrender.com/api";
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    if (interval === "hour") {
      ...
      if (hour >= 0 && hour < 6) timeOfDayFactor = 0.4; // Night - low usage
        ...
        timeOfDayFactor = 0.9; // Morning - medium usage
      else if (hour >= 12 && hour < 18)
        timeOfDayFactor = 1.1; // Afternoon - high usage
      else timeOfDayFactor = 0.8; // Evening - medium-low usage
    }

    // Final calculation with randomization
    const interactions = Math.round(
      baseInteractions * weekendFactor * timeOfDayFactor
    );
    ...
    const revenue = parseFloat(
      (totalTokens * 0.00002 * (0.8 + Math.random() * 0.4)).toFixed(2)
    );
...
const generateFakeSummary = (timeframe, popularityFactor = 1.0) => {
  ...
  const baseInteractions = Math.floor(
    (500 + Math.floor(Math.random() * 1000)) * popularityFactor
  );
  const baseRevenue = Math.floor(
    (50 + Math.floor(Math.random() * 100)) * popularityFactor
  );
  ...
  const retentionRate = parseFloat(
    ((uniqueUsers / interactions) * 100).toFixed(1)
  );
  ...
  const projectedMonthlyRevenue = parseFloat(
    (projectedMonthlyInteractions * revenuePerInteraction).toFixed(2)
  );
...
export const getDeveloperDashboardSummary = async (timeframe = "week") => {
  ...
  try {
    ...
    try {
      ...
      if (
        modelResponse.data?.success &&
        Array.isArray(modelResponse.data.data)
      ) {
    ...
    console.log(
      "No meaningful data in API response, generating fake data with model count:",
      modelCount
    );
    ...
    if (modelCount > 0) {
      ...
      if (
        fakeData.data.modelsPerformance &&
        fakeData.data.modelsPerformance.length !== modelCount
      ) {
        ...
        if (fakeData.data.modelsPerformance.length > modelCount) {
          ...
          fakeData.data.modelsPerformance =
            fakeData.data.modelsPerformance.slice(0, modelCount);
        } else {
          ...
          for (let i = currentLength; i < modelCount; i++) {
            ...
            const newModel = {
              ...fakeData.data.modelsPerformance[i % currentLength],
            };
...
const generateFakeDashboardData = (timeframe, modelCount = 0) => {
  ...
  const modelTemplates = [
    {
      name: "GPT Assistant",
      description: "General purpose AI assistant model",
      category: "Assistants",
    },
    {
      name: "Code Helper",
    ...
    },
    {
      ...
      description: "Content and creative writing AI",
      category: "Content",
    },
    {
      name: "Image Descriptor",
      description: "Visual content analysis and description",
      category: "Vision",
    },
    {
      name: "Data Analyzer",
  ...
  const numModels =
    modelCount > 0 ? modelCount : 3 + Math.floor(Math.random() * 3);
  ...
  while (selectedTemplates.length < numModels) {
    selectedTemplates = selectedTemplates.concat(
      shuffledTemplates.slice(
        0,
        Math.min(shuffledTemplates.length, numModels - selectedTemplates.length)
      )
    );
  }

  // Generate models with varying performance metrics
  const models = selectedTemplates
    .slice(0, numModels)
    .map((template, index) => {
      ...
      const daysPublished = Math.max(
        5,
        modelAge - Math.floor(Math.random() * 10)
      ); // Published 5-10 days after creation
      ...
      const tokenPrice =
        pricingModel === "per-token" ? 0.00001 + Math.random() * 0.00004 : 0;
      const subscriptionPrice =
        pricingModel === "subscription"
          ? 9.99 + Math.floor(Math.random() * 5) * 10
          : 0;
      ...
      return {
        ...
        createdAt: new Date(
          Date.now() - modelAge * 24 * 60 * 60 * 1000
        ).toISOString(),
        publishedAt: new Date(
          Date.now() - daysPublished * 24 * 60 * 60 * 1000
        ).toISOString(),
        stats: {
          ...fakeSummary,
  ...
  const summaryStats = {
    ...
    totalInteractions: models.reduce(
      (sum, model) => sum + model.stats.interactions,
      0
    ),
    totalRevenue: parseFloat(
      models.reduce((sum, model) => sum + model.stats.revenue, 0).toFixed(2)
    ),
    totalTokens: models.reduce(
      (sum, model) => sum + model.stats.tokens.total,
      0
    ),
  };
...
export const getModelDetailedAnalytics = async (
  modelId,
  timeframe = "week",
  interval = "day"
) => {
...
const generateFakeModelAnalytics = (modelId, timeframe, interval) => {
  ...
  const modelTemplates = {
    model1: {
      name: "GPT Assistant",
      description:
        "Versatile AI assistant for general-purpose tasks and conversations",
      category: "Assistants",
      popularity: 1.2,
    },
    model2: {
      name: "Code Helper",
      description:
        "Specialized model for programming assistance and code generation",
      category: "Code",
      popularity: 1.0,
    },
    model3: {
      name: "Creative Writer",
      description:
        "AI model focused on creative content generation and storytelling",
      category: "Content",
      popularity: 0.9,
    },
    model4: {
      name: "Image Descriptor",
      description:
        "Vision model that analyzes and describes image content with precision",
      category: "Vision",
      popularity: 0.8,
    },
    model5: {
      name: "Data Analyzer",
      description:
        "Advanced analytical model for data processing and insights generation",
      category: "Data",
      popularity: 1.1,
    },
    model6: {
      name: "Translation Pro",
      description:
        "Multilingual translation model supporting over 50 languages",
      category: "Language",
      popularity: 0.7,
    },
  };
  ...
  const totalInteractions = timeSeriesData.reduce(
    (sum, item) => sum + item.interactions,
    0
  );
  const totalRevenue = parseFloat(
    timeSeriesData.reduce((sum, item) => sum + item.revenue, 0).toFixed(2)
  );
  const totalInputTokens = timeSeriesData.reduce(
    (sum, item) => sum + item.tokens.input,
    0
  );
  const totalOutputTokens = timeSeriesData.reduce(
    (sum, item) => sum + item.tokens.output,
    0
  );
  const totalTokens = totalInputTokens + totalOutputTokens;
  const totalUniqueUsers = timeSeriesData.reduce(
  ...
  const revenuePerInteraction = parseFloat(
    (totalRevenue / totalInteractions).toFixed(4)
  );
  ...
  const retentionRate = parseFloat(
    ((totalUniqueUsers / totalInteractions) * 100).toFixed(1)
  );
  ...
  if (Math.random() < 0.2) {
    const metric = ["interactions", "revenue", "tokens", "users"][
